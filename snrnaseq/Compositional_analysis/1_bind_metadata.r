...
HBCC1$Age <- as.numeric(HBCC1$Age); HBCC2$Age <- as.numeric(HBCC2$Age)
HBCC1 <- subset(HBCC1, Age > 20 & Age < 70); HBCC2 <- subset(HBCC2, Age > 20 & Age < 70)
...
Multi <- subset(readRDS("/scratch/nendresz/P1_Brain_scope/Files/Multiome.rds"), Age_death < 70 & Disorder %in% c("control","Schizophrenia"))

# Extract metadata
Ruz_McLean <- subset(Ruz, Cohort == "McLean")@meta.data
Ruz_MtSinai <- subset(Ruz, Cohort == "MtSinai")@meta.data
...
Ruz_MtSinai_meta <- collapse_meta(Ruz_MtSinai)
hbcc_meta$Age <- as.numeric(hbcc_meta$Age)
mssm_meta$Age <- as.numeric(mssm_meta$Age)

# Multiome metadata
Multi_meta <- Multi %>% group_by(Donor) %>% summarise(Age=as.numeric(first(Age_death)),Sex=first(Biological_Sex),Diagnosis=first(Disorder),.groups="drop")

# Combine donors
all_meta <- bind_rows(HBCC=hbcc_meta,MSSM=mssm_meta,OFC=ofc_meta,Batiuk=bat_meta,McLean=Ruz_McLean_meta,MtSinai=Ruz_MtSinai_meta,Multiome=Multi_meta,.id="Cohort")
