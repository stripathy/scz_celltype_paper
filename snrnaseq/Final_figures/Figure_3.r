...
final_results <- read.csv("/scratch/nendresz/P1_Compositional_analysis/Files/crumblr_results_final_meta.csv")
...
ggsave("Figures/Barchart_SCZ_meta_FINAL.png", p3a, width = 21, height = 8,  dpi = 600)
...
merged_all <- read.csv("/scratch/nendresz/P1_Compositional_analysis/neuron_props_7_cohorts")
merged_all$cohort <- recode(merged_all$cohort, "Frölich" = "Fröhlich", "MtSinai" = "MSSM 1", "MSSM" = "MSSM 2")

res <- read.csv("/scratch/nendresz/P1_Compositional_analysis/Files/final_results_crumblr_7_cohorts_all.csv")
...
df_xen <- read.csv("/scratch/nendresz/Xenium/xen_Sst_proportions.csv")
res_xen <- read.csv("/scratch/nendresz/Xenium/crumblr_results_supertype_neuronal.csv") %>% filter(celltype == "Sst_25")
...
ggsave("Figures/Boxplots3b_with_Xenium.png", p3b, width = 14, height = 8, dpi = 600)

# Figure 3C
library(dplyr)
library(ggplot2)
plot_data <- read.csv("/scratch/nendresz/P1_Compositional_analysis/plotdata.csv")
res_xen <- read.csv("/scratch/nendresz/Xenium/crumblr_results_supertype_neuronal.csv") %>% filter(celltype == "Sst_25") 
res_xen$CellType <- res_xen$celltype
xen_n <- read.csv("/scratch/nendresz/Xenium/xen_Sst_proportions.csv") %>% filter(subtype=="Sst_25") %>% nrow()
...
ggsave("Figures/Forestplot3c.png",p3c,width=10,height=5,dpi=600)
######
...
library(patchwork)
library(cowplot)
seu <- readRDS("/scratch/nendresz/Xenium/Xenium_SCZ_R.rds")
...
ggsave("Figures/Spatial3d.png",p3d,width=16,height=8,dpi=600,bg="white")
...
res <- read.csv("/scratch/nendresz/Xenium/crumblr_results_supertype_neuronal.csv")
res$CellType <- res$celltype
final_results <- read.csv("/scratch/nendresz/P1_Compositional_analysis/Files/final_results_crumblr_7_cohorts.csv")
...
ggsave("Figures/concordanceplot.png", p3f, width=9.5, height=8, dpi=600)
ggsave("Figures/concordanceplot.svg", p3f, width=9.5, height=8)
...
ggsave("Figures/p3h_Sst_depth_effect.png",p3h,width=8,height=6,dpi=600)
ggsave("Figures/p3h_Sst_depth_effect.svg",p3h,width=8,height=6)
...
ggsave("Figures/Figure3_composite.png", full, width=22, height=24, dpi=900, bg="white")
ggsave("Figures/Figure3_composite.svg", full, width=22, height=24, dpi=900, bg="white")
