...
| snrnaseq → genetics, transcriptomic, snrnaseq/composition_sensitivity | `shared/snrnaseq_de/nicole_scz_snrnaseq_betas/` | **regenerated in place, 2026-09-13** — the original was never staged down from the cluster; `PROVENANCE.md` beside it records how it was rebuilt and what it was checked against |
...
Every chain now executes end to end, but two links are **reconstructions** rather
than recovered originals, and both are labelled as such where they live:

| link | state |
|---|---|
| `shared/snrnaseq_de/nicole_scz_snrnaseq_betas/final_results_crumblr_7_cohorts.csv` | regenerated by re-running the repo's own composition scripts; reproduces the committed per-dataset and FE-meta tables to ~1e-15. **Now tracked in git** — the only file in that directory that is |
...
Open items from the 2026-09-13 audit are in
