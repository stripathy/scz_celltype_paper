"""
Parallel pipeline: label transfer, depth prediction, and plotting.

Full pipeline for all Xenium samples using multiprocessing:
  1. Load raw Xenium data
  2. kNN label transfer from SEA-AD snRNAseq reference
...
def _process_one_sample(sample_info):
    ...
    try:
        ...
        print(f"  [{pid}] {sid}: {n_cells:,} cells, annotating...")

        # Label transfer
        annotated = annotate_sample(adata, _ref)
...
def main():
    parser = argparse.ArgumentParser(
        description="Parallel Xenium pipeline: label transfer + depth prediction"
    )
    parser.add_argument("--data_dir", required=True,
                        help="Directory with Xenium .h5 and boundary files")
    parser.add_argument("--reference", required=True,
                        help="SEA-AD reference h5ad path")
