...
build_exemplar <- function(gene, dx, lim, tab, base,
  ...
  sz_count <- mm(base - 0.5)    # 6.5 pt at BASE 7
  sz_bar   <- mm(base - 0.5)    # 6.5 pt (was 0.26*BASE = 1.8 pt: the illegible one)
  sz_lab   <- mm(base - 1)      # 6 pt, the tick-label tier

  p <- ggplot() +
    geom_polygon(data = bd, aes(x, y), fill = "grey93", colour = "grey40", linewidth = 0.35) +
    geom_polygon(data = nu, aes(x, y), fill = NA, colour = "grey25", linewidth = 0.3,
                 linetype = "22") +
    { if (nd > 0) geom_point(data = dt, aes(x, y), colour = DOT_COL, size = 0.55, alpha = 0.9) } +
    # 5 um bar, bottom right of every cell
    annotate("segment", x = lim * 0.96 - SB_UM, xend = lim * 0.96,
             y = -lim * 0.94, yend = -lim * 0.94, linewidth = 0.9, lineend = "square") +
...
exemplar_key <- function(gene, base) {
  sz <- (base - 1) / .pt        # tick-label tier, matching the outline labels
  ggplot() +
    annotate("segment", x = 0.02, xend = 0.09, y = 0.5, yend = 0.5, colour = "grey40", linewidth = 0.5) +
    ...
    annotate("point", x = 0.60, y = 0.5, colour = DOT_COL, size = 0.9) +
...
ex_pair <- function(gene, lim, tab, base, show_hdr,
  ...
  hts  <- c(0.16, 1)
  if (key != "none") { rows <- c(rows, list(if (key == "draw") exemplar_key(gene, base) else ggdraw()))
                       hts  <- c(hts, 0.14) }
