...
| 26 | The repo uses **Git LFS** (23 files; the large per-cohort DE tables) and said so nowhere. A clone without `git lfs pull` gets 134-byte pointer stubs, and **nothing errors** — `read.csv()` on a pointer returns a two-row frame of pointer text, so `Final_figures/Supplemental/Code/Supertype_DE.r` (Supplementary S4) would run to completion on garbage. Only the figure-input manifest catches it, by refusing to draw Figure 2 and reporting its source as changed, which is how this was found on 2026-09-16. `README.md` now documents the requirement and how to spot a stub. Worth deciding whether the four LFS files that ship in the working tree should be fetched as part of setup. | repo-wide |
